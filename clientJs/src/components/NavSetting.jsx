...
function NavSetting() {
    ...
    const {adminDetails, clearAdminDetails, mode, setmode} = useStore()
    ...
    const StyledMenu = styled((props) => (
        <Menu
          ...
          {...props}
          sx={{border: mode === 'light' ? `0.5px solid ${palette.strokeMain}` : '1px solid rgba(255, 255, 255, 0.12)',
          }}
        />
      ))(({ theme }) => ({
        top:'-10px',
    ...
    <Stack sx={{width:'100%', textAlign:'right'}} direction={'row'} alignItems="center">
      ...
      >
        <Stack sx={{borderBottom:'1px solid rgba(0, 17, 102, 0.1)', }}>
          <span className='mt-1 mx-2 px-2 pt-1' style={{fontFamily: 'Inter, sans-serif', fontSize:'14px', color:mode==='light'? palette.txtStrong : 'white', }}>{adminDetails.first_name + " " + adminDetails.last_name}</span>
          <span className='mb-2 mt-1 mx-2 px-2 pb-2' style={{fontFamily: 'Inter, sans-serif', fontSize:'12px', color:mode==='light'?palette.textWeak: 'white'}}>{"ID: " + adminDetails.admin_id}</span>
        </Stack>
